...
import numpy as np
import glob
from Static_Feature_Extraction.Schaefer_features.Classification_feature import region_reho_average, region_alff_average
from Static_Feature_Extraction.Schaefer_features.Classification_feature import FC_extraction
...
for j in range(len_rbd):
    schaefer_data.loc[j] = [result_rbd[j], ALFF_RBD[j], ReHo_RBD[j], fALFF_RBD[j], 1]

for k in range(len_hc):
    schaefer_data.loc[len_rbd + k] = [result_hc[k], ALFF_HC[k], ReHo_HC[k], fALFF_HC[k], 0]

schaefer_data.to_pickle('schaefer_200_non_PCA.pkl')
