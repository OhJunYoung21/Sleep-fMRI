...
def extract_data(c, param_map):
    ...
    if not anat_base:
        msg = "Anatomical Extracted_Data template incorrect. No such file or directory %s", anat_base
        logging.exception(msg)
        raise Exception(msg)

    if not func_base:
        msg = "Functional Extracted_Data template incorrect. No such file or directory %s, func_base"
        logging.exception(msg)
        raise Exception(msg)
...
def generate_supplementary_files(data_config_outdir, data_config_name):
    ...
    try:
        ...
        err = "\n\n[!] Extracted_Data configuration file couldn't be read!\nFile " \
